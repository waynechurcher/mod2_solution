...
/* loop drawing of elements newBarL
          ...
          for (var i=0; i<barsL.length;i++){
            ...
            p.rect(showBar.pos, -p.windowHeight, showBar.width*3, p.windowHeight*3);
            p.pop();
            barsL[i].pos += vel1;
...
*/
          for (var i=0; i<barsR.length;i++){
            ...
            barsR[i].pos -= vel2*(Math.abs(p.windowWidth-(p.windowWidth-barsR[i].pos))/p.windowWidth);
            //console.log(vel1, barsR.length-1);
            if (barsR.length>200){
              barsR.splice(i,1);//taking out element 1 at a time to manage the array
